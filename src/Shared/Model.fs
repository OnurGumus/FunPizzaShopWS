...
type Price =
    ...
    static member TryCreate(s: decimal) =
        single (fun t -> t.TestOne s |> t.Gte 0m Negative |> t.Map Price |> t.End)
...
module Pizza =
    ...
    type Pizza = {
        ...
        member this.BasePrice =
            (this.Size |> decimal) / (Pizza.DefaultSize |> decimal)
            * this.Special.BasePrice.Value
            |> Price.TryCreate
            |> forceValidate
    ...
    type OrderId =
        ...
        member this.Value = let (OrderId orderId) = this in orderId

        static member CreateNew() =
            "Order_" + Guid.NewGuid().ToString()
    ...
    type DeliveryId =
        ...
        member this.Value = let (DeliveryId deliveryId) = this in deliveryId

        static member CreateNew() =
            "Delivery_" + Guid.NewGuid().ToString()
            |> ShortString.TryCreate
            |> forceValidate
            |> DeliveryId
    ...
    type Order = {
        ...
        member this.TotalPrice =
            this.Pizzas
            |> List.sumBy (fun p -> p.TotalPrice.Value)
            |> Price.TryCreate
            |> forceValidate
